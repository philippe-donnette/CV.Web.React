...
describe("src/components/experience/experience-carousel.jsx", function() {
  ...
  describe("Tests with experience items", () => {
  ...
  describe("Tests without any experience items", () => {
    ...
    it("should not render CarouselIndicators component", () => {
      let component = shallowResult.find(CarouselIndicators);
      expect(component.length).to.be.equal(0);    
    });
  
    it("should not have a previous nav button", () => {
      let a = shallowResult.find('a').findWhere(x => x.props()['data-slide'] === 'prev');
      expect(a.length).to.be.equal(0);    
    });

    it("should not have a next nav button", () => {
      let a = shallowResult.find('a').findWhere(x => x.props()['data-slide'] === 'next');
      expect(a.length).to.be.equal(0);    
    });
